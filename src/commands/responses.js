...
export async function handleResponses(args, { client }) {
    ...
    if (action === "get") {
        const id = rest[0];
        if (!id) {
            return { ok: false, error: "client_error", message: "Usage: agentsuit responses get <id>" };
        }

        const result = await client.request({
            method: "GET",
            route: `/v1/responses/${encodeURIComponent(id)}`,
        });
    ...
    if (action === "delete") {
        const id = rest[0];
        if (!id) {
            return { ok: false, error: "client_error", message: "Usage: agentsuit responses delete <id>" };
        }
    ...
    return {
        ok: false,
        error: "client_error",
        message: "Usage: agentsuit responses <list|create|get|delete>",
    };
}
